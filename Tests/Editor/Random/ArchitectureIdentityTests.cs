...
namespace MakeIt.Random.Tests
{
	class ArchitectureIdentityTests
	{
		...
		{
			string dataFileName = string.Format(crossPlatform ? dataFilePathTemplate_CrossPlatform : dataFilePathTemplate, typeof(TRandom).Name, operationName);
			string dataFilePath = Path.GetFullPath("Packages/com.andygainey.makeit.random/Tests/Editor/Random/Data~/");
			try
			{
				using (var file = File.Open(Path.Combine(dataFilePath, dataFileName), FileMode.Open, FileAccess.Read))
				{
